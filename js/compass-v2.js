window.LW=window.LW||{};
LW.compass=(()=>{
 const HUD_ID='compassHUD',TICKS_ID='compassTicks',READOUT_ID='compassReadout',PX_PER_DEG=5.2;let heading=null,listening=false;
 ...
 function renderOne(h,root){const el=root.querySelector('.compass-ticks');buildTicks(el);if(el)el.style.transform=`translateX(${-h*PX_PER_DEG}px)`;const r=root.querySelector('.compass-readout');if(r){const deg=Math.round(h)%360;r.textContent=`${label(deg)||''} ${String(deg).padStart(3,'0')}°`.trim()}}
 ...
 function handle(e){let h=null;if(typeof e.webkitCompassHeading==='number'&&isFinite(e.webkitCompassHeading))h=normalize(e.webkitCompassHeading);else if((e.absolute||e.type==='deviceorientationabsolute')&&typeof e.alpha==='number'&&isFinite(e.alpha))h=normalize(360-e.alpha);if(h===null)return;heading=h;render(h)}
