...
LW.reticle={
 ...
 targetFrame(ctx,cx,cy,w,h,c,active){const rh=Math.max(h*.20,180),rw=rh*45/76,x=cx-rw/2,y=cy-rh/2;ctx.save();ctx.strokeStyle=c;ctx.globalAlpha=active?.98:.8;ctx.lineWidth=Math.max(2,w/900);ctx.setLineDash(active?[]:[10,8]);ctx.strokeRect(x,y,rw,rh);ctx.setLineDash([]);const p=Math.max(7,w/180);[[x,y],[x+rw,y],[x,y+rh],[x+rw,y+rh]].forEach(q=>{ctx.beginPath();ctx.arc(q[0],q[1],p,0,Math.PI*2);ctx.fill()});ctx.font=`${Math.max(12,w/110)}px monospace`;ctx.textAlign='center';ctx.fillText('76 cm',cx,y-Math.max(10,w/90));ctx.save();ctx.translate(x-Math.max(12,w/90),cy);ctx.rotate(-Math.PI/2);ctx.fillText('45 cm',0,0);ctx.restore();ctx.restore()},
