...
LW.image={
  thermal(v){
    if(v<.2){let t=v/.2;return[0,60*t,150+105*t]}
    if(v<.4){let t=(v-.2)/.2;return[0,60+195*t,255-180*t]}
    if(v<.6){let t=(v-.4)/.2;return[255*t,255,75*(1-t)]}
    if(v<.8){let t=(v-.6)/.2;return[255,255-180*t,0]}
    let t=(v-.8)/.2;return[255,75*t,75*t]
  },
  process(data,s){
    for(let i=0;i<data.length;i+=4){
      ...
      else {let q=y*255;data[i]=data[i+1]=data[i+2]=q}
    }
  }
};
