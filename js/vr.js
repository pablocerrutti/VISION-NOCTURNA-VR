...
LW.vr={
 ...
 drawOverlay(ctx,w,h,s,p){
   ...
   const lp={...s,point:p?{x:p.x/2,y:p.y}:null},rp={...s,point:p?{x:ew+gap+p.x/2,y:p.y}:null};
   ctx.save();ctx.beginPath();ctx.rect(0,0,ew,h);ctx.clip();LW.reticle.draw(ctx,ew,h,lp);ctx.restore();
   ctx.save();ctx.beginPath();ctx.rect(ew+gap,0,ew,h);ctx.clip();LW.reticle.draw(ctx,ew+gap,h,rp);ctx.restore();
 }
};