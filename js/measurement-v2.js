...
LW.measure={
  ...
  verticalFov(viewportWidth=innerWidth,viewportHeight=innerHeight,zoom=1){
    ...
    if(viewportAspect>sourceAspect)vf=2*Math.atan(Math.tan(hf/2)/viewportAspect);
    const z=Math.max(0.9,Number(zoom)||1),effectiveZoom=Math.max(1,z);
  ...
  distanceFromPitchDegrees(pitch){
    const a=Math.abs(Number(pitch));
    if(!Number.isFinite(a))return this.outOfRange();
    return this.distanceFromAngle(a*Math.PI/180);
  },
  angleFromScreenY(y,viewportHeight=innerHeight,zoom=1,viewportWidth=innerWidth){
    ...
    const n=y/h,offset=2*(n-.5);if(offset<=0)return 0;
