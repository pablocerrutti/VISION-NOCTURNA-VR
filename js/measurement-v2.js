...
LW.measure={
 ...
 min:10,max:100,preferredAccuracyMax:70,
 ...
 distanceFromTwoAngles(baseAngle,topAngle){if(!Number.isFinite(baseAngle)||!Number.isFinite(topAngle))return this.outOfRange();const delta=Math.abs(Math.tan(topAngle)-Math.tan(baseAngle));if(delta<=1e-8)return this.outOfRange();const d=this.targetHeight/delta;if(!Number.isFinite(d))return this.outOfRange();if(d<this.min)return{value:null,text:'OBJETO DEMASIADO CERCANO',code:'TOO_CLOSE',estimated:d,baseAngle,topAngle,delta};if(d>this.max)return{value:null,text:'OBJETO SUPERA RANGO',code:'OVER_RANGE',estimated:d,baseAngle,topAngle,delta};return{value:d,text:d.toFixed(1)+' M',code:d<=this.preferredAccuracyMax?'HIGH_ACCURACY':'ESTIMATED',baseAngle,topAngle,delta}},
 ...
 outOfRange(){return{value:null,text:'FUERA DE RANGO',code:'OUT_OF_RANGE'}}
};
