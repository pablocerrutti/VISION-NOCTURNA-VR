...
LW.measure={
  ...
  verticalFov(viewportWidth=innerWidth,viewportHeight=innerHeight,zoom=1){
    ...
    const z=Math.max(0.9,Number(zoom)||1),effectiveZoom=Math.max(1,z);return 2*Math.atan(Math.tan(vf/2)/effectiveZoom)*180/Math.PI;
  },
  ...
  distanceFromAngle(angle){
    ...
    if(!Number.isFinite(d)||d<this.min||d>this.max)return this.outOfRange();
