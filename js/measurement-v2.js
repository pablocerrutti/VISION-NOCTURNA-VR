...
LW.measure={
  ...
  verticalFov(viewportWidth=innerWidth,viewportHeight=innerHeight,zoom=1){
    ...
    let vf;
    if(sourceAspect>viewportAspect)vf=2*Math.atan(Math.tan(hf/2)/viewportAspect);
    else vf=2*Math.atan(Math.tan(hf/2)/sourceAspect);
    const z=Math.max(1,Number(zoom)||1);
    return 2*Math.atan(Math.tan(vf/2)/z)*180/Math.PI;
  },
  ...
  angleFromScreenY(y,viewportHeight=innerHeight,zoom=1,viewportWidth=innerWidth){
    ...
    const n=y/h,offset=2*(n-.5);if(offset<=0)return null;
